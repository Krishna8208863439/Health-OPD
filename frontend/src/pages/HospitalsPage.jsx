    ...
    <div className="w-full max-w-[1700px] mx-auto px-4 sm:px-6 lg:px-10 py-8 space-y-8 animate-fade-in">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-200 pb-6">
        <div>
          ...
          <h1 className="text-2xl sm:text-3xl font-extrabold text-slate-900">
            Find Nearby Hospitals & Emergency Centers
          </h1>
      ...
      <div className="p-4 sm:p-5 rounded-3xl bg-white border border-slate-200 shadow-sm flex flex-col md:flex-row items-center justify-between gap-4">
        ...
        <form onSubmit={handleSearch} className="relative w-full md:w-96">
          <input
            ...
            className="w-full bg-slate-50 border border-slate-300 rounded-xl pl-9 pr-4 py-2.5 text-xs text-slate-900 focus:outline-none focus:border-cyan-600 focus:bg-white"
          />
      ...
      {loading ? (
        ...
        <div className="p-12 text-center bg-white rounded-3xl border border-slate-200 space-y-2">
          ...
          <p className="text-xs text-slate-500">Try adjusting your search keywords or city filter.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
          {hospitals.map((h) => (
            <div
              key={h.id}
              className="p-5 rounded-3xl bg-white border border-slate-200/90 hover:border-cyan-400 hover:shadow-md transition space-y-4 flex flex-col justify-between"
            >
              <div className="space-y-2.5">
                ...
                <div className="grid grid-cols-2 gap-2 pt-2 border-t border-slate-100">
                  <div className="p-2.5 rounded-2xl bg-slate-50 border border-slate-100 text-center">
                    <div className="text-[10px] text-slate-500 font-mono font-semibold">Total Beds</div>
                    <div className="text-sm font-bold text-slate-900 font-mono flex items-center justify-center gap-1 mt-0.5">
                      <Bed className="w-3.5 h-3.5 text-cyan-600" /> {h.beds}
                    </div>
                  </div>

                  <div className="p-2.5 rounded-2xl bg-slate-50 border border-slate-100 text-center">
                    <div className="text-[10px] text-slate-500 font-mono font-semibold">Available ICU</div>
                    <div className="text-sm font-bold text-emerald-700 font-mono flex items-center justify-center gap-1 mt-0.5">
              ...
              <div className="flex items-center gap-2 pt-2">
                <a
                  href={`tel:${h.phone}`}
                  className="flex-1 py-2.5 rounded-xl bg-cyan-50 hover:bg-cyan-600 hover:text-white text-cyan-800 font-semibold text-xs transition flex items-center justify-center gap-1.5 border border-cyan-200 shadow-sm"
                >
