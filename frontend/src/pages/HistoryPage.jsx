    ...
    <div className="max-w-6xl mx-auto py-8 px-4 space-y-8 animate-fade-in">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-800 pb-6">
        <div>
          ...
          <h1 className="text-2xl sm:text-3xl font-extrabold text-white">Diagnostic History & Records</h1>
        ...
        <button
          onClick={loadData}
          className="px-3.5 py-2 rounded-xl bg-slate-900 hover:bg-slate-800 border border-slate-700 text-slate-300 text-xs font-semibold flex items-center gap-1.5 transition self-start sm:self-auto"
        >
      ...
      <div className="p-4 rounded-2xl bg-slate-900/70 border border-slate-800 flex flex-col md:flex-row items-center justify-between gap-4">
        ...
        <div className="flex flex-wrap items-center gap-3 w-full md:w-auto">
          ...
          <div className="flex items-center gap-2">
            <span className="text-xs text-slate-400 font-mono">Disease:</span>
            <select
              value={diseaseFilter}
          ...
          {/* Risk Level Filter */}
          <div className="flex items-center gap-2">
            <span className="text-xs text-slate-400 font-mono">Risk Tier:</span>
            <select
              value={riskFilter}
      ...
      <div className="p-6 rounded-2xl bg-slate-900/60 border border-slate-800 backdrop-blur-xl">
        {loading ? (
          <div className="py-12 flex flex-col items-center justify-center space-y-3">
            ...
            <span className="text-xs text-slate-400 font-mono">Querying patient records...</span>
          </div>
        ) : predictions.length === 0 ? (
          <div className="py-12 text-center space-y-3">
            <AlertCircle className="w-8 h-8 text-slate-500 mx-auto" />
            <p className="text-slate-300 text-sm font-semibold">No Assessment Records Found</p>
                ...
                {predictions.map((pred) => {
                  ...
                  return (
                    <tr key={pred.id} className="hover:bg-slate-800/30 transition">
                      <td className="py-3.5 px-3 text-slate-400 font-bold">#{pred.id}</td>
                      <td className="py-3.5 px-3 font-sans font-semibold capitalize text-slate-200">
